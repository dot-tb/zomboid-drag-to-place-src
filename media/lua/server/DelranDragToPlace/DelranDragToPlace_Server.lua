...
function ISPlace3DItemCursor:isValid(square)
    if DragToPlace.placingItem and DragToPlace.options.faceItemWhilePlacing then
        -- The isValid function makes the player turn towards the item we are placing.
        original_isvalid(self, square);
    end
    -- Disable restrictions on where you can place an item.
    return true;
end
